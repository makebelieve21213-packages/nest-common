...
### Исправлено
- RolesGuard: теперь использует `user.role` (строка) вместо `user.roles` (массив). Корректная работа с проектами, где в `request.user` роль передаётся как единственная строка.
